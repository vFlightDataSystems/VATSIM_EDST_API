...
def _get_airport(airport):
    ...
    airport_data = client.navdata.airports.find_one({'icao': airport}, {'_id': False})
    return jsonify(airport_data)


@navdata_blueprint.route('/airport/<airport>/procedures')
def _get_airport_procedures(airport):
    client = g.mongo_nav_client
    data = list(client.navdata.procedures.find({'airports': {'$elemMatch': {'airport': airport}}}, {'_id': False}))
    for row in data:
...
def _get_airway(airway):
    client = g.mongo_nav_client
    airway_data = list(client.navdata.airways.find({'airway': airway}, {'_id': False}))
...
def _get_waypoint(waypoint):
    client = g.mongo_nav_client
    waypoint_data = list(client.navdata.waypoints.find({'waypoint_id': waypoint}, {'_id': False}))
...
def _get_procedure(procedure):
    client = g.mongo_nav_client
    procedure_data = client.navdata.procedures.find_one({'procedure': procedure}, {'_id': False})
