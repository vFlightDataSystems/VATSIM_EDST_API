...
def _request_profile():
    ...
    if facility := profile.facility and profile.name:
        client: MongoClient = g.mongo_adapt_client
        client.adaptationProfiles.requests[facility.lower()].insert(vars(profile))
...
def _get_profile(facility):
    client: MongoClient = g.mongo_adapt_client
    profile = client.adaptationProfiles[facility].find_one({}, {'_id': False})
